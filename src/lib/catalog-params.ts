...
const KNOWN_CUISINES: Cuisine[] = ["cafe", "italian", "indian", "fusion"];
const KNOWN_DIETARY: DietaryTag[] = ["veg", "vegan", "gluten-free", "contains-nuts", "halal", "spicy", "chefs-special", "seasonal", "bestseller", "new"];
const VALID_SPICE_MAX = new Set([0, 1, 2, 3]);
...
export function parseProductQuery(sp: URLSearchParams): ProductQuery {
  ...
  const cuisine = csv("cuisine").filter((c) => KNOWN_CUISINES.includes(c as Cuisine)) as Cuisine[];
  const dietary = csv("dietary").filter((d) => KNOWN_DIETARY.includes(d as DietaryTag)) as DietaryTag[];
  ...
  if (sp.get("spiceMax")) {
    const n = Number(sp.get("spiceMax"));
    if (VALID_SPICE_MAX.has(n)) q.spiceMax = n as ProductQuery["spiceMax"];
  }
